...
\title{Class Notes for \emph{Mathematical Introduction to the Finite Element Method}, taught by Tonatiuh Sánchez-Vizuet at Univ. of Arizona}
\author{Jack Murphy: Created from lecture notes, supplementary notes, and homework solutions}
\date{Spring 2026}
...
\begin{remark}
To do: in class we recorded a Banach fixed point proof: define the operator $A:V\to V^*$ by $(Au)(v)=a(u,v)$, convert the problem to $Au=\ell$, and construct a contraction for a suitable relaxation parameter. This perspective is especially useful later when dealing with nonsymmetric perturbations.
\end{remark}
